...
namespace lua {
	...
	// Get
	template <typename T>
	std::enable_if_t<std::is_enum_v<T> || std::is_integral_v<T> || std::is_floating_point_v<T>, T> getNumber(lua_State* L, int32_t arg, T defaultValue = {}) {
		int isnum;
	...
	// Push
	template <typename T>
	std::enable_if_t<std::is_enum_v<T> || std::is_integral_v<T> || std::is_floating_point_v<T>, void> pushNumber(lua_State* L, T value) {
