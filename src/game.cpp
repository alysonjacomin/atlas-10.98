...
Creature* Game::getCreatureByName(const std::string& s) {
	...
	if (auto it = std::find_if(mappedPlayerNames.begin(), mappedPlayerNames.end(), predicate); it != mappedPlayerNames.end()) {
		return it->second;
	}

	if (auto it = std::find_if(npcs.begin(), npcs.end(), predicate); it != npcs.end()) {
		return it->second;
	}

	if (auto it = std::find_if(monsters.begin(), monsters.end(), predicate); it != monsters.end()) {
		return it->second;
	}
...
void Game::playerMoveItem(Player* player, const Position& fromPos, uint16_t spriteId, uint8_t fromStackPos, const Position& toPos, uint8_t count, Item* item, Thing* toThing) {
	if (!player->canDoAction()) {
		uint32_t delay = player->getNextActionTime();
	...
	if (item->isHangable() && toThingTile->hasFlag(TILESTATE_SUPPORTS_HANGABLE)) {
		...
		if (!playerPos.isInRange(mapToPos, 1, 1, 0)) {
			...
			if (fromPos.x != 0xFFFF && mapFromPos.isInRange(playerPos, 1, 1) && !mapFromPos.isInRange(walkPos, 1, 1, 0)) {
				...
				ReturnValue ret = internalMoveItem(fromThing, player, INDEX_WHEREEVER, item, count, &moveItem, 0, player, nullptr, &fromPos, &toPos);
				if (ret != RETURNVALUE_NOERROR) {
					player->sendCancelMessage(ret);
					return;
				}
	...
	ReturnValue ret = internalMoveItem(fromThing, toThing, toIndex, item, count, nullptr, 0, player, nullptr, &fromPos, &toPos);
	if (ret != RETURNVALUE_NOERROR) {
		player->sendCancelMessage(ret);
	}
}

ReturnValue Game::internalMoveItem(Thing* fromThing, Thing* toThing, int32_t index, Item* item, uint32_t count, Item** _moveItem, uint32_t flags /*= 0*/, Creature* actor /* = nullptr*/, Item* tradeItem /* = nullptr*/, const Position* fromPos /*= nullptr*/, const Position* toPos /*= nullptr*/) {
	Player* actorPlayer = actor ? actor->getPlayer() : nullptr;
	if (actorPlayer && fromPos && toPos) {
		const ReturnValue ret = events::player::onMoveItem(actorPlayer, item, count, *fromPos, *toPos, fromThing, toThing);
		if (ret != RETURNVALUE_NOERROR) {
			return ret;
		}
	}
	...
	if (ret == RETURNVALUE_NEEDEXCHANGE) {
		...
		if (ret == RETURNVALUE_NOERROR) {
			if (actorPlayer && fromPos && toPos) {
				const ReturnValue eventRet = events::player::onMoveItem(actorPlayer, toItem, toItem->getItemCount(), *toPos, *fromPos, toThing, fromThing);
			...
			ReturnValue retExchangeMaxCount = fromThing->queryMaxCount(INDEX_WHEREEVER, *toItem, toItem->getItemCount(), maxExchangeQueryCount, 0);
			...
			if (toThing->queryRemove(*toItem, toItem->getItemCount(), flags, actor) == RETURNVALUE_NOERROR) {
				...
				if (actorPlayer && fromPos && toPos && !toItem->isRemoved()) {
					events::player::onItemMoved(actorPlayer, toItem, toItem->getItemCount(), *toPos, *fromPos, toThing, fromThing);
				}
	...
	if (actorPlayer && fromPos && toPos) {
		if (updateItem && !updateItem->isRemoved()) {
			events::player::onItemMoved(actorPlayer, updateItem, count, *fromPos, *toPos, fromThing, toThing);
		} else if (moveItem && !moveItem->isRemoved()) {
			events::player::onItemMoved(actorPlayer, moveItem, count, *fromPos, *toPos, fromThing, toThing);
		} else if (item && !item->isRemoved()) {
			events::player::onItemMoved(actorPlayer, item, count, *fromPos, *toPos, fromThing, toThing);
		}
	}
...
ReturnValue Game::internalAddItem(Thing* toThing, Item* item, int32_t index, uint32_t flags, bool test, uint32_t& remainderCount) {
	if (!toThing || !item) {
		return RETURNVALUE_NOTPOSSIBLE;
	}
...
Item* Game::findItemOfType(Thing* fromThing, uint16_t itemId, bool depthSearch /*= true*/, int32_t subType /*= -1*/) const {
	if (!fromThing) {
		return nullptr;
	}
...
void Game::playerDebugAssert(uint32_t playerId, const std::string& assertLine, const std::string& date, const std::string& description, const std::string& comment) {
	...
	if (file) {
		...
		fprintf(file, "----- %s - %s (%s) -----\n", timestamp.c_str(), player->getName().c_str(), player->getIP().to_string().c_str());
